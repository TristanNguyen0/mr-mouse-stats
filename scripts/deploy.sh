...
if [[ "$TARGET" == "all" || "$TARGET" == "frontend" ]]; then
  ...
  # Only _next/ is content-hashed, so only it may cache forever. Everything
  # outside it keeps a stable name across builds: the HTML, and — less
  # obviously — the RSC payloads (players/__next._tree.txt and friends). Those
  # carry the buildId, so pinning them makes a returning browser disagree with
  # the HTML it just fetched, and Next answers a buildId mismatch with a full
