...
| `github_owner_id` | `99143948` | Numeric account ID of the owner. GitHub puts it in the OIDC subject claim, so the trust condition needs it too. `curl -s https://api.github.com/repos/<owner>/<name> \| jq .owner.id` |
| `github_repository_id` | `1318869120` | Numeric ID of the repository, same reason. `curl -s https://api.github.com/repos/<owner>/<name> \| jq .id` |
...
| Deploy workflow: `Not authorized to perform sts:AssumeRoleWithWebIdentity` | The ARN resolved but the token's `sub` doesn't match the trust condition. Don't guess at the `sub` — read the one actually presented, which CloudTrail records even though the call failed: `aws cloudtrail lookup-events --max-results 1 --lookup-attributes AttributeKey=EventName,AttributeValue=AssumeRoleWithWebIdentity --query 'Events[0].Username' --output text`. Compare it against `aws iam get-role --role-name mr-mouse-stats-github-deploy --query 'Role.AssumeRolePolicyDocument'`. Usual causes: `github_owner_id`/`github_repository_id` unset or stale (the claim carries numeric IDs, not just names), the job not declaring `environment: production`, or `github_repository` not matching the real `owner/name`. |
...
| Deploy workflow: `Could not assume role with OIDC: Request ARN is invalid` | The variable *is* set, but the value is not a well-formed role ARN, so STS rejects it before the trust policy is ever consulted — nothing is wrong in AWS. Usual causes: the surrounding quotes from a `terraform output` without `-raw`, a stray leading/trailing space or newline from the copy-paste, the role *name* instead of the ARN, or the `oidc-provider` ARN in place of the `role` one. Re-copy from `terraform -chdir=infra output -raw github_deploy_role_arn`; it must read `arn:aws:iam::<account-id>:role/mr-mouse-stats-github-deploy` and nothing else. |
